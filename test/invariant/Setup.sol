...
abstract contract Setup is Test, Permit2User, WithSalts, TestConstants {
    ...
    function setup() internal {
        ...
        bytes memory baselineSaltArgs = abi.encodePacked(
            ...
            abi.encode(
                address(_baselineAuctionHouse), address(_kernel), address(_quoteToken)
            )
        );
        ...
        _dtlBaseline.configureDependencies();
    ...
    function _deployBPOOL(
        ...
        bytes32 salt = _getSalt(
            kernel_, _name, _symbol, _decimals, _factory, _reserve, _feeTier, _initialActiveTick
        );
