...
abstract contract UniswapV3DTLHandler is BeforeAfter, Assertions {
    ...
    function uniswapV3DTL_createLot(
        ...
        vestingStart =
            uint48(bound(uint256(vestingStart), uint48(block.timestamp) + 2 days, uint256(type(uint48).max)));
    ...
    struct OnSettleProfitTemps {
        address attacker;
        address seller;
        uint96 lotId;
    }

    // function UniswapV3DTL_OnSettleProfit(uint256 attackerIndexSeed, uint256 lotIndexSeed) public {
    //     // PRE-CONDITIONS
    //     OnSettleProfitTemps memory d;
    //     d.attacker = randomAddress(attackerIndexSeed);
    //     d.lotId = randomLotIdV3(lotIndexSeed);
    //     (d.seller,,,,,,,,) = _auctionHouse.lotRouting(d.lotId);

    //     if (d.attacker == d.seller) return;

    //     vm.warp(block.timestamp + 86_401);

    //     givenAddressHasQuoteTokenBalance(address(_auctionHouse), 20 ether);
    //     givenAddressHasBaseTokenBalance(d.attacker, 10 ether);
    //     givenAddressHasBaseTokenBalance(d.seller, 10 ether);
    //     givenAddressHasBaseTokenAllowance(d.seller, _dtlV3Address, 10 ether);

    //     uint160 sqrtPriceX96 = 100e35;

    //     bool quoteTokenIsToken0 = address(_quoteToken) < address(_baseToken);
    //     address token0 = quoteTokenIsToken0 ? address(_quoteToken) : address(_baseToken);
    //     address token1 = quoteTokenIsToken0 ? address(_baseToken) : address(_quoteToken);

    //     address pool = _uniV3Factory.getPool(token0, token1, _poolFee);

    //     if (pool == address(0)) {
    //         pool = _uniV3Factory.createPool(token0, token1, _poolFee);
    ...
    //     _batchAuctionModule.setLotSettlement(d.lotId, 10 ether, 10 ether, true);

    //     // ACTION
    //     vm.prank(d.attacker);
    //     try _auctionHouse.settle(
    //         d.lotId, 1, abi.encode("")
    //     ) {
    //         vm.prank(d.attacker);
    //         _baseToken.approve(address(_v3SwapRouter), 10 ether);

    //         ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
    //             tokenIn: address(_baseToken),
    //             tokenOut: address(_quoteToken),
    //             fee: 500,
    //             recipient: d.attacker,
    //             deadline: block.timestamp,
    //             amountIn: _baseToken.balanceOf(address(pool)),
    //             amountOutMinimum: 0,
    //             sqrtPriceLimitX96: 0
    //         });

    //         vm.prank(d.attacker);
    //         try _v3SwapRouter.exactInputSingle(params) {
    //             // POST-CONDITIONS

    //             t(
    //                 false,
    //                 "AX-53: Profit should not be extractable due to UniswapV3Pool price manipulation"
    //             );
    //         } catch {
    //             return;
    //         }
    ...
    function setV3CallbackParameters(uint96 lotId, uint96 proceeds_, uint96 refund_) internal {
        ...
        _quoteTokensToDepositV3 =
            _proceedsV3 * lotIdCreationParamsV3[lotId].poolPercent / 100e2;
